...
#define OBJECTALIGNED __attribute__((aligned (8)))
#define BUFFERSIZE 36                       // Number of bits+4

#if defined(BOARD_SIPEED_LONGAN_NANO)
  #define WORKSPACESIZE (3516) //can not go higher
  #define STACKDIFF 0
#else
...
PGM_P const streamname[] PROGMEM = {serialstream, i2cstream, spistream, sdstream, stringstream,gfxstream};

// Typedefs

typedef intptr_t symbol_t;
typedef uintptr_t builtin_t;
typedef intptr_t chars_t;
...
enum builtins: builtin_t { 
  NIL, TEE, NOTHING, OPTIONAL, AMPREST, LAMBDA, LET, LETSTAR, CLOSURE, QUOTE, DEFUN, DEFVAR, EQ,
  CAR, FIRST, CDR, REST, NTH, CHAR, STRINGFN, PINMODE, DIGITALWRITE, REGISTER, FORMAT, 
};
...
#endif
#if defined(gfxsupport)
inline void gfxwrite (char c) { lcd.write(c); }
#endif
/*
...
object *eval (object *form, object *env) {
  ...
  if ((uintptr_t)sp - (uintptr_t)__bss_end < STACKDIFF) { Context = NIL; error2(PSTR("stack overflow")); }
